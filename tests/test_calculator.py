...
"""
    ...
    print("Actual output from help test:", captured.out)
...
def test_display_history_with_entries(capsys):
    ...
    # Arrange
    history = [
        "AddOperation: 10.0 Addition 5.0 = 15.0",
        "SubtractOperation: 20.0 Subtraction 3.0 = 17.0",
        "MultiplyOperation: 7.0 Multiplication 8.0 = 56.0",
        "DivideOperation: 20.0 Division 4.0 = 5.0"
    ]
...
1. AddOperation: 10.0 Addition 5.0 = 15.0
2. SubtractOperation: 20.0 Subtraction 3.0 = 17.0
3. MultiplyOperation: 7.0 Multiplication 8.0 = 56.0
4. DivideOperation: 20.0 Division 4.0 = 5.0"""
    assert captured.out.strip() == expected_output.strip()
...
def test_calculator_addition(monkeypatch, capsys):
    ...
    # Assert
    captured = capsys.readouterr()
    print("Actual output:", captured.out)
    assert "Result: AdditionOperation: 10.0 Addition 5.0 = 15.0" in captured.out
...
def test_calculator_subtraction(monkeypatch, capsys):
    ...
    # Assert
    captured = capsys.readouterr()
    assert "Result: SubtractOperation: 20.0 Subtract 5.0 = 15.0" in captured.out
...
def test_calculator_multiplication(monkeypatch, capsys):
    ...
    # Assert
    captured = capsys.readouterr()
    assert "Result: MultiplyOperation: 7.0 Multiply 8.0 = 56.0" in captured.out
...
def test_calculator_division(monkeypatch, capsys):
    ...
    # Assert
    captured = capsys.readouterr()
    assert "Result: DivideOperation: 20.0 Divide 4.0 = 5.0" in captured.out
...
def test_calculator_history(monkeypatch, capsys):
    ...
    print("Actual output:", captured.out)
    assert ">> Result: AdditionOperation: 10.0 Addition 5.0 = 15.0" in captured.out
    assert ">> Result: SubtractOperation: 20.0 Subtract 3.0" in captured.out
    assert ">> Calculation History:" in captured.out
    assert "1. AdditionOperation: 10.0 Addition 5.0 = 15.0" in captured.out
    assert "2. SubtractOperation: 20.0 Subtract 3.0 = 17.0" in captured.out
...
def test_unsupported_operation_max(monkeypatch, capsys):
    user_input = 'max 2 3\nexit\n'
    monkeypatch.setattr('sys.stdin', StringIO(user_input))
    with pytest.raises(SystemExit):
        calculator()
    captured = capsys.readouterr()
    print("Captured output:", repr(captured.out))
    assert "Unsupported operation type: 'max'" in captured.out
...
def test_unsupported_operation_max(monkeypatch, capsys):
    ...
    with pytest.raises(SystemExit):
        calculator()
    captured = capsys.readouterr()
    print("Actual output:", repr(captured.out))
    # Updated assertion matching the actual message
    assert "Unsupported operation type: 'max'" in captured.out
...
def test_calculator_unexpected_exception(monkeypatch, capsys):
    ...
    monkeypatch.setattr('app.calculation.CalculationFactory.create_operation', mock_create_calculation)
    user_input = 'add 10 5\nexit\n'
    monkeypatch.setattr('sys.stdin', StringIO(user_input))
